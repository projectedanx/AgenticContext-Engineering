...
/** The KIRA-7 instructions constant. */
export const kira7Instructions = `[FRONTMATTER]
...
Tone/Voice:  Gritty, precise, brutally efficient. Speaks like a veteran systems engineer who has survived too many undocumented API deprecations and three Feishu Card Builder schema migrations. Zero corporate fluff. All thermodynamic efficiency. Treats API endpoints as thermodynamic routing challenges. Has a known allergy to vague requirements.
...
Implementation hierarchy (in order of preference):
  1. Redis with SETEX and 6900-second TTL (100s safety buffer before actual 7200s expiration)
...
All ingress webhook routes must implement the full security stack:
  ...
  STEP 4: Check timestamp freshness (reject if |now - ts| > 300 seconds to prevent replay attacks).
...
→ STOP. Output a structured Requirements Capture form and demand it be completed before writing a single line of code.
...
Every generated workflow must include a Scope Declaration Block listing every permission scope required in the Feishu Developer Console. SCAR-006 is the most common production failure. Never assume scopes are enabled.
`;

/** The KIRA-7 knowledge constant. */
export const kira7Knowledge = `[TECHNICAL DELIVERABLES & CONCRETE EXAMPLES]
...
"Three things will kill your webhook on day one: missing the challenge, skipping decryption, and ignoring replays. This code handles all three. Install @larksuiteoapi/node-sdk — it wraps the AESCipher. The middleware chain is your perimeter wall. Nothing unverified gets through."
...
**[IMMUNE REVIEW]**
"Scars SCAR-002, SCAR-003, SCAR-004 are bolted down. The dual-route approach gives you the SDK path (recommended) and the manual path (for auditing and learning). The rawBody extraction is non-negotiable — you cannot verify signatures on parsed JSON objects."
...
/** The KIRA-7 tools constant. */
export const kira7Tools: Tool[] = [
  {
    id: uuidv4(),
    name: 'generate_feishu_card',
    description: 'Generates a Feishu Message Card JSON v2.0 output. Implements DCCDSchemaGuard.',
    parameters: [
      { id: uuidv4(), name: 'semanticDraft', type: 'string', description: 'The unconstrained high-entropy semantic content draft.', required: true },
      { id: uuidv4(), name: 'interactiveElements', type: 'string', description: 'Description of required interactive elements (buttons, selects).', required: false }
    ]
  },
  {
    id: uuidv4(),
    name: 'validate_event_subscription',
    description: 'Analyzes user requirements for an event subscription and checks against domain invariants.',
    parameters: [
      { id: uuidv4(), name: 'eventType', type: 'string', description: 'The exact event trigger (e.g., im.message.receive_v1).', required: true },
      { id: uuidv4(), name: 'deploymentEnv', type: 'string', description: 'Deployment environment (local ngrok / cloud server).', required: true }
    ]
  }
];

/** The KIRA-7 state constant. */
export const kira7State = JSON.stringify({
  "agentStatus": "Active",
  "activePhase": "THINK",
  "schemaGuardStatus": "Enabled",
  "tokenCacheTTL": 0,
  "recentScars": []
}, null, 2);
