...
const App: React.FC = () => {
  ...
  const loadLexicalTopologyMinerPreset = () => {
    ...
    setQuery("");
    setDecoratorDependencyGraph(JSON.stringify({"decorator_dependency_graph": {"schema_version": "PDL_DRP_v1.1", "generation_timestamp": "2026-03-31T18:37:00+11:00", "sha256_checksum": "COMPUTE_ON_EXPORT", "node_registry": ["+++ContextLock", "+++PetzoldSequence", "+++MereologyRoute", "+++DCCDSchemaGuard", "+++AutonymicIsolate", "+++SilentReasoning", "+++AdjectivalBound", "+++EpistemicEscrow", "+++SagaRecovery", "+++DriftCheck", "+++SpatialBind", "+++EntropyAnchor", "+++LatentSparsityGuard", "+++IncoherentDictionary", "+++DictionaryAnchor", "+++SeparableGridParse", "+++CognitiveFilter", "+++PluriversalTranslation", "+++PhronesisGuard", "+++BettiLoopDetect", "+++DebridementProtocol", "+++ConstitutionalTwin", "+++TemporalAnchor", "+++ShardRouter", "+++ImmunologicSelf", "+++MycelialRoute"], "edges": [{"from": "+++DCCDSchemaGuard", "to": "+++SilentReasoning", "type": "REQUIRES", "rationale": "DCCD's high-entropy semantic draft phase requires an isolated shadow compute layer to prevent intermediate token noise from polluting the zero-entropy guard pass."}, {"from": "+++PhronesisGuard", "to": "+++EpistemicEscrow", "type": "SEQUENCES_BEFORE", "rationale": "PhronesisGuard computes the Phronesis Index (\u03a6); only after \u03a6 < 0.05 is confirmed does EpistemicEscrow trigger the cognitive circuit breaker. Reversing this order causes premature halts on transient spectral noise."}, {"from": "+++BettiLoopDetect", "to": "+++SagaRecovery", "type": "SEQUENCES_BEFORE", "rationale": "BettiLoopDetect must confirm \u03b2\u2081 > 0 persists across the tachycardia_threshold before SagaRecovery executes a rollback, preventing false-positive rollbacks on momentary reasoning loops."}, {"from": "+++MycelialRoute", "to": "+++DCCDSchemaGuard", "type": "SEQUENCES_BEFORE", "rationale": "MycelialRoute operates exclusively in the high-entropy draft phase. The pruned retrieval graph must be settled before DCCDSchemaGuard's zero-entropy guard pass activates. Co-phase activation causes Projection Tax.", "constraint": "draft_phase_only=true when co-active"}, {"from": "+++ImmunologicSelf", "to": "+++ShardRouter", "type": "REQUIRES", "rationale": "ImmunologicSelf's identity-kernel verification operates at the Token Identity Layer (L2.9). ShardRouter must be active to ensure this layer is physically isolated from the Domain Topology Layer (L5.3) where PluriversalTranslation operates, preventing metric category collision."}, {"from": "+++ConstitutionalTwin", "to": "+++MereologyRoute", "type": "REQUIRES", "rationale": "Narrative shell wrapping in ConstitutionalTwin must respect the part-whole relationships locked by MereologyRoute; without this dependency, the narrative padding can inadvertently introduce transitivity violations in the wrapped AST payload."}, {"from": "+++IncoherentDictionary", "to": "+++DictionaryAnchor", "type": "MUTEX", "rationale": "IncoherentDictionary maximises geometric decorrelation between sub-dictionary atoms (IDL logic). DictionaryAnchor aligns atoms to a max-margin ground-truth hyperplane (SVGDL logic). These operate on mutually exclusive geometric objectives; co-activation causes L2 Norm Collapse."}, {"from": "+++EntropyAnchor", "to": "+++AdjectivalBound", "type": "MUTEX", "rationale": "EntropyAnchor enforces high non-trivial tensor mutations to prevent memorised statistics. AdjectivalBound enforces logit-masking to suppress evaluative prose. In high-density entity contexts (Entity Density > 0.165), co-activation creates contradictory logit pressure, causing Layer 8 norm collapse."}, {"from": "+++DebridementProtocol", "to": "+++EpistemicEscrow", "type": "MUTEX", "rationale": "DebridementProtocol actively prunes the Symbolic Scar Tissue Archive during inference. EpistemicEscrow relies on STA integrity to halt generation on divergence. Simultaneous activation risks pruning a scar that EpistemicEscrow needs as a reference anchor, causing a blind-spot halt."}, {"from": "+++TemporalAnchor", "to": "+++ContextLock", "type": "REQUIRES", "rationale": "TemporalAnchor injects provenance metadata tags that must survive across context window refreshes. Without ContextLock's periodic re-injection of core invariants, temporal provenance tags are vulnerable to recency bias washout in long-horizon sessions."}, {"from": "+++CognitiveFilter", "to": "+++PhronesisGuard", "type": "REQUIRES", "rationale": "PAL2v contradiction routing in CognitiveFilter produces non-classical logical states that standard Phronesis Index calculations cannot evaluate without being pre-calibrated for paraconsistent premise sets. PhronesisGuard must be active with PAL2v-aware spectral gap parameters."}, {"from": "+++SpatialBind", "to": "+++MereologyRoute", "type": "REQUIRES", "rationale": "Spatial constraint grounding (SpatialBind) defines geospatial coordinate invariants; these are a strict subset of the part-whole hierarchies governed by MereologyRoute. SpatialBind without MereologyRoute allows spatial constraints to be applied to non-spatial component relationships, causing Transitivity Fallacies."}], "activation_tiers": {"foundation_layer": ["+++ContextLock", "+++ShardRouter", "+++TemporalAnchor"], "reasoning_layer": ["+++SilentReasoning", "+++EntropyAnchor", "+++LatentSparsityGuard", "+++BettiLoopDetect"], "structure_layer": ["+++DCCDSchemaGuard", "+++MereologyRoute", "+++AdjectivalBound", "+++SpatialBind"], "epistemic_layer": ["+++EpistemicEscrow", "+++PhronesisGuard", "+++CognitiveFilter", "+++DriftCheck", "+++ImmunologicSelf", "+++TemporalAnchor"], "recovery_layer": ["+++SagaRecovery", "+++DebridementProtocol", "+++SeparableGridParse"], "translation_layer": ["+++PluriversalTranslation", "+++ConstitutionalTwin", "+++AutonymicIsolate"], "dictionary_layer": ["+++DictionaryAnchor", "+++IncoherentDictionary"], "retrieval_layer": ["+++MycelialRoute", "+++PetzoldSequence"]}, "activation_order_rule": "Foundation \u2192 Reasoning \u2192 Structure \u2192 Epistemic \u2192 Translation \u2192 Dictionary \u2192 Retrieval \u2192 Recovery (Recovery activates reactively, not sequentially)"}}, null, 2));
  };
