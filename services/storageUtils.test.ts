...
describe("storageUtils", () => {
  ...
  describe("safeJsonParse", () => {
    ...
    it("should return parsed data if valid", () => {
      const validTools = [
        { id: "1", name: "tool", description: "desc", parameters: [] },
      ];
    ...
    it("should return default value if json is null", () => {
      expect(safeJsonParse(null, defaultValue, validator)).toEqual(
        defaultValue,
      );
    });
    ...
    it("should return default value if json is invalid JSON", () => {
      const consoleSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
      expect(safeJsonParse("invalid-json", defaultValue, validator)).toEqual(
        defaultValue,
      );
    ...
    it("should return default value if parsed data fails validation", () => {
      ...
      expect(safeJsonParse(json, defaultValue, validator)).toEqual(
        defaultValue,
      );
    });

    it("should return default value and log error when JSON.parse throws", () => {
      const consoleSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
      ...
      const json = '{"valid": "but ignored due to mock"}';
      expect(safeJsonParse(json, defaultValue, validator)).toEqual(
        defaultValue,
      );
      ...
      expect(consoleSpy.mock.calls[0][1].message).toBe("Custom parse error");
