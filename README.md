...
11. **Cognitive Civil Engineering (The Dual-Helix Harness):** Transitioning from pure conversational models to deterministic execution requires fusing Reflexion (verbal metacognition) with Voyager (executable skill synthesis). Reflexion is leveraged to calculate the Martensite Initiation Quotient (MIQ) and document "Symbolic Scars" (routing around failure nodes). Voyager is utilized to compile successful tracebacks into C2PA-compliant executable primitives. This Dual-Helix architecture balances Cognitive Viscosity (token overhead of deep reflection) with Latency/Throughput, entirely eradicating Skill Drifting and Heuristic Fossilization over deep execution horizons.

10. **ts-morph Syntax Encapsulation**: When programmatically generating JSDoc comments via `ts-morph`, passing raw formatted strings (e.g., `*\n * Description`) bypasses the AST's internal comment wrapper and results in malformed, double-asterisk syntax errors that break downstream parsers. The strictly enforced methodology is to utilize the `JSDocStructure` interface (passing an object with `description` and `tags` arrays) via `insertJsDoc` or `addJsDoc`, which guarantees structural compliance and preserves exact file indentation.
